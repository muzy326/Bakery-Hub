...
async function rateItem(id: string, rating: number) {
  const res = await fetch(`/api/menu/${id}/rate`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ rating }),
  });
  if (!res.ok) throw new Error("Failed to submit rating");
  return res.json() as Promise<{ success: boolean; averageRating: number; ratingCount: number }>;
}

export default function MenuCard({ item, onAddToCart, inCart }: MenuCardProps) {
  const [avgRating, setAvgRating] = useState(item.averageRating ?? null);
