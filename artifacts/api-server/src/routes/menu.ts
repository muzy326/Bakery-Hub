...
router.post("/menu/:id/rate", (req, res) => {
  const item = menuItems.find((i) => i.id === req.params.id);
  if (!item) {
    res.status(404).json({ error: "Item not found" });
    return;
  ...
  item.ratings.push(parsed.data.rating);
  const avg =
    Math.round(
      (item.ratings.reduce((a, b) => a + b, 0) / item.ratings.length) * 10,
    ) / 10;

  res.json({ success: true, averageRating: avg, ratingCount: item.ratings.length });
});
